...
Для production-развёртываний MCP используйте **обёртку с автоматическим перезапуском**, чтобы сервер оставался активным во время обновлений без прерывания сессии клиента.
...
1. Обёртка запускает `seia mcp` в цикле `while true`.
2. Если процесс завершается, он перезапускается в течение 0,2 секунды.
3. Для обновления: `kill $(pgrep -f "seia mcp" | head -1)`
